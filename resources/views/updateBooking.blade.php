...
<body>
    ...
    <div class="container">
        ...
        <div class="booking-info">
                ...
                <span id="service-price">{{$booking->price}} {{$booking->currency}}</span>
                ...
                <form id="bookingForm" action="{{route('formDetails')}}" method="GET">
                    ...
                    <input type="hidden" name="service_duration" value="{{$booking->service_duration}}">
                    <input type="hidden" name="service_price" value="{{$booking->price}}">
                    <input type="hidden" name="service_currency" value="{{$booking->currency}}">
                </form>
        </div>
...
// Pre-select values from the Booking model
const preSelectedDate = "{{ $booking->date ?? '' }}"; // Add the field in your Booking model
const preSelectedTime = "{{ $booking->time ?? '' }}"; // Add the field in your Booking model

let currentDate = new Date(); // Start in the current month
...
function updateCalendar() {
    ...
    for (let day = 1; day <= lastDayOfMonth.getDate(); day++) {
        ...
        if (dayDiv.getAttribute('data-date') === preSelectedDate) {
            dayDiv.classList.add('selected');
            selectDate(dayDiv, true); // Pass true to avoid re-populating time slots unnecessarily
        }
...
function selectDate(dayDiv, isInitialSelection = false) {
    // Check if the day has a point marker
    if (dayDiv.querySelector('.point')) {
        // If it has a point, show timepicker and hide design-placeholder
        ...
        if (!isInitialSelection || date !== preSelectedDate) {
...
function populateTimeSlots(date) {
    ...
    function renderTimeSlots() {
        ...
        times.slice(0, slotsToShow).forEach(time => {
            ...
            if (time === preSelectedTime && date === preSelectedDate) {
                slotDiv.classList.add('selected');
                selectedTimeInfo.textContent = `Tijd: ${slotDiv.textContent}`;
                appointmentDetails.textContent = `Afspraak op ${new Date(date).toLocaleDateString('nl-NL')} om ${slotDiv.textContent}`;
                nextButton.classList.remove('disabled');
